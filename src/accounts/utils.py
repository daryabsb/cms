...
def nodes_per_page():
    ...
    nodes_per_page = int(
        load_config_data['Reading']['nodes_per_page']['value'])
    return int(Configurations.objects.get(name='Pagination.nodes_per_page').value)
...
def data_filter(filter_form_data, model_obj):
    ...
    if mobile:
        if filter_data:
            ...
            filter_data = model_obj.objects.filter(
                phone_number__icontains=mobile)
    if group:
        if filter_data:
