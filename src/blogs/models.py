...
class Blogs(models.Model):
    ...
    status = models.CharField(
        max_length=255, choices=PAGE_STATUS_CHOICES, default=PAGE_STATUS_CHOICES[0][0])
    visibility = models.CharField(
        max_length=255, choices=PAGE_VISIBILITY_CHOICES, default=PAGE_VISIBILITY_CHOICES[0][0])
    ...
    feature_image = models.ImageField(
        upload_to=upload_image_file_path, max_length=500, default="default/default.jpg")
    video_url = models.URLField(
        null=True, blank=True, help_text="YouTube Embed URL")
    # Automatically set the field to now when the object is first created.
    created = models.DateTimeField(auto_now_add=True)
    # Automatically set the field to now every time the object is saved.
    updated = models.DateTimeField(auto_now=True)
...
class Seo(models.Model):
    ...
    blog = models.OneToOneField(
        Blogs, on_delete=models.CASCADE, primary_key=True)
