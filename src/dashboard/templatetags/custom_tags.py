...
from src.blogs.models import Blogs, Categories, Tags
...
def getAllprefix(var):
    name_list = Configurations.objects.all().order_by(
        'created_at').values_list('name', flat=True)
    prefixes = []
...
def getRecentPOST(count):
    blogs = Blogs.objects.filter(status='Published').exclude(
        visibility='Pr').order_by('-publish_on')[:int(count)]
    return blogs
...
def getWidgetArchives(val):
    ...
    arc_list = []
    month_temp_list = []
    archive_list = Blogs.objects.filter(status='Published').exclude(
        visibility='Pr').values_list("publish_on", flat=True)
