# Generated by Django 5.0.3 on 2024-05-21 03:20

import django.db.models.expressions
from django.db import migrations, models


class Migration(migrations.Migration):

    dependencies = [
        ('hud', '0029_posorder_discount_sign_posorder_discounted_amount'),
    ]

    operations = [
        migrations.AddField(
            model_name='posorder',
            name='subtotal_after_discount',
            field=models.GeneratedField(db_persist=True, expression=models.Case(models.When(discount_type=1, then=django.db.models.expressions.CombinedExpression(models.F('item_subtotal'), '-', django.db.models.expressions.CombinedExpression(models.F('item_subtotal'), '*', django.db.models.expressions.CombinedExpression(models.F('discount'), '/', models.Value(100))))), models.When(discount_type=0, then=django.db.models.expressions.CombinedExpression(models.F('item_subtotal'), '-', models.F('discount'))), default=models.Value(0), output_field=models.DecimalField(decimal_places=3, default=0, max_digits=15)), output_field=models.DecimalField(decimal_places=3, default=0, max_digits=15)),
        ),
    ]
